...
export async function generateMetadata(
  ...
  const product: ProductType = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/products/by-name/${name}`, {cache: 'force-cache'}).then(res => res.json());
  const title = `${product.name} - ${product.brand}`;
  const description = `Buy ${product.name} from ${product.brand}. Price: ${product.price.toFixed(2)} zł. Available in various colors: ${product.colors.map(color => color.name).join(', ')}.`;
  return {
    title,
    description,
    openGraph: {
